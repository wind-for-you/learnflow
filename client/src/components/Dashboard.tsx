...
import { goalApi, checkinApi, taskApi, authApi } from '../services/api';
...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      const [goalsResponse, checkinsResponse, tasksResponse, statsResponse, todayResponse] =
        await Promise.all([
      ...
      console.error('加载今日数据失败:', loadErr);
  ...
  const primaryPlanId = todayTasks.find((task) => task.planId)?.planId
    || recentTasks.find((task) => task.planId)?.planId;
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* 页面头部 */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">今日学习</h1>
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            {user?.name}，先完成今天的任务，再记录一次打卡
          </p>
        </div>
          ...
          <div className="card">
            <div className="card-body">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">连续打卡</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{currentStreak} 天</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            <div className="card-body">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">总学习时长</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalStudyTime} 小时</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <Link to={primaryPlanId ? `/plans/${primaryPlanId}` : '/plans'} className="card hover:shadow-md transition-shadow duration-200">
            <div className="card-body text-center">
              <BookOpenIcon className="h-8 w-8 text-primary-600 mx-auto mb-2" />
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">继续计划</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">查看并完成任务</p>
            </div>
          </Link>
          ...
          <Link to="/goals/new" className="card hover:shadow-md transition-shadow duration-200">
            <div className="card-body text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">创建目标</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">设定新的学习方向</p>
            </div>
          </Link>
        </div>
        ...
        <div className="card mb-8">
          <div className="card-body">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">{todayTasks.length} 个待办</span>
            </div>
            ...
            {todayTasks.length > 0 ? (
              <div className="space-y-2">
                {todayTasks.slice(0, 3).map((task, index) => (
                  <div key={task.id} className="flex items-start p-2 bg-gray-50 dark:bg-gray-800 rounded text-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-gray-900 dark:text-white font-medium line-clamp-1">{task.title}</div>
                ...
                <div className="text-center pt-2">
                  <Link
                    to={primaryPlanId ? `/plans/${primaryPlanId}` : '/plans'}
