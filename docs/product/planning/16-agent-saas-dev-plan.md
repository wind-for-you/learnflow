...
版本：v1.2  
状态：待评审  
**上游产品文档**：`15-toc-saas-rice-backlog.md`（v1.4+，与本文 §8.1 结论对齐）  
...
### 3.1 管理后台与「运行配置中心」（Wave 4 前产品补足）

当前仅有面向终端用户的能力，**缺少内部管理后台**；AI 与 Agent 相关参数长期散落在环境变量与代码中，不利于多厂商切换、代理地址与模型管理。**本段写死为与 Wave 4 并行或插档于 Wave 4 之前的最小交付包**（详见 **Wave 3.5**）。

| 能力 | 说明 | 与工程的关系 |
| --- | --- | --- |
| **Ops / Admin 外壳** | 独立路由（如 `/ops` 或子域），**仅 `role=admin/ops` 或 IP 白名单 + 特性开关** 可访问；首版可简化为「配置页 + 只读监控」 | 不与 to C 主站混排敏感操作 |
| **运行变量 / 配置中心** | 将 **可安全页面化** 的项抽到后台：**各 LLM 厂商 `baseURL`（含自建代理）**、**默认模型 ID**、**超时与重试**、**功能开关**；**API Key / Secret** 仅存服务端（加密字段或引用密钥管理服务），**禁止**以明文回显给浏览器 | 首版读取仍以 **环境变量为权威默认**（见下条），页面保存为 **DB 覆盖或 env 并列策略** 须在实现时二选一并文档化 |
| **阿里百炼（默认链路）** | **当前阶段 AI 相关服务默认以阿里百炼（DashScope 兼容 OpenAI 式调用）环境变量为准**，例如 `DASHSCOPE_API_KEY`、可选 `DASHSCOPE_BASE_URL` / 区域 endpoint；OpenRouter 等保留为 **可切换 Profile** | 与 `server/.env.example`、实现代码对齐；多厂商表结构可后续扩展 |
| **AI 服务管理页** | 同一后台内维护 **多 Profile**（如：`bailian-prod`、`openrouter-dev`）：代理地址、模型名、是否启用、**配额/限流策略键**（与 Wave 7 计量打通） | 复盘、计划生成、周报等调用统一走 **Provider 抽象** |
| **Agent 运维页** | 与 AI 配置复用鉴权：展示 **队列名、积压深度、成功/失败率**、**最近失败任务 ID 与可读错误**；可调 **Worker 并发、单任务超时**（与 env 一致或覆盖） | 解决仅日志中出现 `error: {}`、无法运营的痛点；详见 **§10** |

**边界**：不产生「把用户数据批量导出给第三方」的非合规能力；法务与密钥轮换流程见 `15` DoR。
...
### Wave 3.5 — 管理后台与运行配置中心（插档 Wave 4 前，对齐 `15` 新增条目）

**目标**：补足 **Ops 控制台** + **AI/Agent 可调参数页面化**，默认链路绑定 **阿里百炼相关环境变量**，其余厂商/代理作为可切换 Profile。

| 项 | Agent 动作 | 验证 |
| --- | --- | --- |
| 鉴权与入口 | Ops 路由、管理员角色或 env 开关；无密钥写死在前端 bundle | 非管理员 403；生产默认关 |
| 运行配置模型 | DB 表或等价：`providerProfile`（厂商标识、`baseURL`、默认模型、`timeoutMs`、启用开关）；Secret 仅存服务端加密或外部 Secret 引用 | 单测：更新后下一轮请求生效或可配置「热reload」文档说明 |
| 百炼默认值 | **未配置 DB 覆盖时**，读 `DASHSCOPE_API_KEY`（及可选 `DASHSCOPE_BASE_URL`/区域）作为默认 Provider；复盘/计划/Agent LLM 步骤统一可走该默认 | `.env.example` 与 README 明示；staging 冒烟 |
| AI 管理 UI | 列表 + 编辑：代理/Base URL、模型、超时；**密钥仅「已配置/未配置/轮换」占位输入** | 手工：切换模型后调用成功/回退可观测 |
| Agent 运维 UI | 队列健康、积压、最近失败摘要；日志级错误可读（对标 §10） | Worker 人造失败时 UI 能看到非空原因 |
| 与 Wave 7 | 预埋 **配额策略键** 或与 Redis 计数 key 对齐，避免二次推翻 | Code review 勾选 |
...
### 7.1 已知运行现象（非「服务未启动」，与可观测性）

| 现象 | 含义 | 建议 |
| --- | --- | --- |
| 日志：`AI复盘生成失败，使用模板回退 timeout of 20000ms exceeded` | 外呼 LLM 超过当前超时阈值，业务已 **降级到模板**，**不等于**端口或 DB 挂了 | 调大超时、检查网络/代理/密钥；Wave 3.5/W7 将此暴露为后台可配项 |
| 日志：`Agent Worker 执行失败` 且 `error: {}` | 多为 **错误对象未被序列化**，真实原因在栈或 Axios 包装的 `cause` 中 | **工程补强**：结构化 `serializeError` + 写入任务表 `lastError`；Wave 3.5 UI 可读 |

---

## 8. 须你（产品/负责人）确认的信息

**Q1–Q6 的结论见 §8.1。** **Q7** 为 Wave 3.5 新增（AI 默认厂商与后台配置边界）。若变更支付渠道或存储厂商，须更新 §8.1 并重新评审 Wave 5/8。
...
| **Q7** | **AI 运行时默认链路：阿里百炼（DashScope）**（`DASHSCOPE_*`）；其他厂商/OpenRouter **仅作为可切换 Provider Profile**；Secrets **不经由前端明文** | 2026-05-02 | 待评审会签字 |
...
| 2026-05-02 | v1.2 | §3.1 管理后台与运行配置中心；**Wave 3.5**（Ops、AI Profile、百炼 env 默认、Agent 运维 UI）；Mermaid `W3→W3.5→W4`；§5 映射；**§7.1** 复盘超时与 Agent `error:{}`；§8.1 **Q7**（百炼默认 Provider）；**§10** env 命名建议 |

---

## 10.（附录）运行配置环境变量命名建议（与 Wave 3.5 对齐）

以下供实现与 `server/env.example` 对齐，**以代码实际为准**；产品要求 **默认走阿里百炼**。

| 变量 | 用途 |
| --- | --- |
| `DASHSCOPE_API_KEY` | 百炼 / DashScope 调用密钥（主默认） |
| `DASHSCOPE_BASE_URL`（可选） | 自定义网关或区域 endpoint |
| 既有 `OPENROUTER_API_KEY` 等 | 保留为 **Profile** 切换，不强制删除 |

DB 中保存的 `baseURL`/模型等 **覆盖** env 默认时，须在后台注明优先级与审计。
